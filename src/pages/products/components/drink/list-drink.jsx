...
import svgOrangeJuice from "../../../../assets/img/orange-juice.svg"
import {changeStatusShowInfoProduct} from '../../../../redux/products/action/index'
import { getIdDrink } from '../../../../redux/products/action/index';
...
const ListDrink = () => {
    ...
    const handleClick = (e) => {
        ...
        dispatch(getIdDrink(id))
    }
    const filterProduct = listDrinks.filter((value) => {
    ...
    return (
        <div className="bg-gray-100" style={{minHeight:"85vh"}}>
            <div className=" mt-28 mt-32-mobile relative">
                ...
                {isStatusShowInfoProduct?<DrinkInfo/>:null}
                    <main className="lg:col-span-5 w-2/3 m-auto">
                        <div className="mt-16">
                            <div className="flex justify-center">
                                <span className="font-bold text-2xl md:text-4xl">Drink</span>
                                <img src={svgOrangeJuice} alt="svgHumburger" className="h-6 w-6 md:h-8 md:w-8 ml-2 md:ml-5 inline-flex" />
                            </div>
                                            ...
                                            <div key={key} id={food.id} onClick={handleClick} className="bg-white rounded-lg shadow-md p-7 hover:shadow-lg item" style={{maxHeight:"454px"}}>
                                                    <img src={food.image} alt="burger" className=" w-full max-h-72 rounded-3xl transform hover:scale-105 transition duration-300" />
                                                        <div className=" mt-5 text-center">
                                                            <span className="font-bold text-2xl truncate overflow-hiden block">{food.name}</span>
                                                ...
                                                <div key={key} id={food.id} onClick={handleClick} className="bg-white rounded-lg shadow-md p-7 hover:shadow-lg item" style={{maxHeight:"454px"}}>
                                                        <img src={food.image} alt="burger" className="w-full max-h-72 rounded-3xl transform hover:scale-105 transition duration-300" />
                                                            <div className=" mt-5 text-center">
                                                                ...
                                                                <div className="flex justify-between text-center mt-5">
                                                                    <span className="block text-custom-yellow  font-bold text-md"><i className="fas fa-tags"></i> {food.price} Đ</span>
                                                                    <span className="block text-custom-yellow font-bold text-"> <i className="fas fa-star"></i> {food.rate}</span>
                                                            </div>
                                                        </div>
                                                </div>  
                                            )
